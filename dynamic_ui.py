...
from core_operations import validate_email, validate_phone, format_phone
...
def display_contacts_dynamic(contacts: List[tuple], detailed: bool = False):
    ...
    if detailed:
        ...
        # Compact view - show first 5 columns or all if <= 5
        display_cols = columns[:min(5, len(columns))]
        ...
        # Show note if there are more columns
        if len(columns) > 5:
            print(f"\nℹ️  Showing {len(display_cols)} of {len(columns)} columns. Use detailed view to see all fields.")
